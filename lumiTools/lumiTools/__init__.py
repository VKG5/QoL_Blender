...
PROPS = [
    ...
    ("vizNode", bpy.props.EnumProperty( name = 'Materials ',
    ...
    ## Render Properties                          
    ("rendererViewSamples", bpy.props.IntProperty( name = 'Viewport Samples ',
                                          description = 'Viewport Sample Count',
                                          default = 32, min = 0, max = 4096)),
                                            
    ("rendererFinalSamples", bpy.props.IntProperty( name = 'Render Samples ',
                                          description = 'Render Sample Count',
                                          default = 128, min = 0, max = 4096)),
]
...
PROPSWorld = [
        ## Visualization Options
        ("worldNode", bpy.props.EnumProperty( name = 'Materials ',
                                          description = 'Pick the visualization mode you want to see',
                                          items = [
...
class changeRendererC(bpy.types.Operator):
    bl_idname = "opr.change_renderer_cycles"
    bl_label = "Switch renderer to Cycles"
    bl_description = "Switch renderer to Cycles without visiting the properties panel"
    
    def execute(self, context):
        basicFuncs.renderEQuality('CYCLES', context.scene.rendererViewSamples, context.scene.rendererFinalSamples)
        
        return {'FINISHED'}
    
class changeRendererE(bpy.types.Operator):
    bl_idname = "opr.change_renderer_eevee"
    bl_label = "Switch renderer to Eevee"
    bl_description = "Switch renderer to Eevee without visiting the properties panel"
    
    def execute(self, context):
        basicFuncs.renderEQuality('BLENDER_EEVEE', context.scene.rendererViewSamples, context.scene.rendererFinalSamples)
        
        return {'FINISHED'}
    
class purgeUnwanted(bpy.types.Operator):
    bl_idname = "opr.orphans_purge_custom"
    bl_label = "Purge unwanted data"
    bl_description = "Clear all orphaned data-blocks without any users in the file"
    
    def execute(self, context):
        basicFuncs.purgeData()
...
#            row.prop(context.scene, prop_name)
        ...
        col.operator("opr.used_to_clear_scene", text="Clear Scene (Deep Clean)", icon = 'DISCLOSURE_TRI_RIGHT')
        ...
        # Get the add-on file path
        col.operator("opr.clear_the_asset", text="Clear Asset(s)", icon = 'REMOVE')
        
        col = self.layout.column(align = True)
        # Get the add-on file path
        col.scale_y = 2
        col.operator("opr.orphans_purge_custom", text="Purge", icon = 'ERROR')
...
class vizUI(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.label(text="Visualization Materials : ")
        
        col = layout.column(align = True)
            
        # Registering the drop-down that contains a list of nodes available
        for(prop_name, _) in PROPS[:1]:
        ...
        if(context.scene.vizNode == 'ao'):
            ...
            layout.label(text="Please visit the shader editor for this.")
        
        col = layout.column(align = True)
        
        if(context.scene.vizNode != 'none'):
            col.operator("opr.set_mat_geo", text = "Update Material(s)", icon = 'ERROR')
        
        col.operator("opr.remove_viz_nodes", text = "Remove Material(s)")
        
        col.separator()
        
        ##============ Quick World ================
        layout.label(text="Quick World : ")
        ...
        ##============ Render Switch ================
        col.separator()
        layout.label(text="Renderer Switch : ")
        col = layout.column(align = True)
        
        # Registering the sample options
        for(prop_name, _) in PROPS[1:]:
            row = col.row(align = True)
            row.prop(context.scene, prop_name)
        ...
        row = col.row()
        row.scale_y = 1.5
        row.operator("opr.change_renderer_cycles", text = "Cycles")
        
        sub = row.row()
        sub.operator("opr.change_renderer_eevee", text = "Eevee")
